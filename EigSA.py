...
#            +1j*np.random.normal(size=(M,N)))*np.sqrt(0.5)
        ...
        x=eigv[:,np.argsort(-np.real(lam))[0]].reshape(M,1)
        pre_x=0
        while np.linalg.norm(pre_x-x)>threshold:
            ...
            rho=Htx[N-K]
            diff_rho=float("inf") 
            while abs(diff_rho)>threshold:
                temp=np.exp(Beta*(IniHtx-rho))
                alg=temp/(1+temp)
                diff_rho=K-np.sum(alg)
                diff2_rho=np.sum(Beta*alg/(1+temp))
                rho=rho-diff_rho/diff2_rho
