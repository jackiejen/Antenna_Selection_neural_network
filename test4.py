import EigHSA,HSA,EigSA,SA,EigSPCA,SPCA
import numpy as np
...
count=1000
TransN=[8,16,32]
Arg=['EigHSA','HSA','EigSA','SA','EigSPCA','SPCA','AAx','NN']
ArgNum=len(Arg)
...
for N in TransN:
    ...
    mPath="D:/Ant/model5/model%s_%s"%(N,M)
    xcol=M*N
    ycol=N
    hiddenSize1=4*N
    hiddenSize2=4*N
...
plt.plot(X,averSNR[Arg.index('EigHSA'),:],'kd-',label="EigHSA")
...
# plt.plot(X,averSNR[Arg.index('AA'),:],'c<-',label="AA")
plt.plot(X,averSNR[Arg.index('NN'),:],'r^-',label="NN")
...
plt.plot(X,averTime[Arg.index('EigHSA'),:],'kd-',label="EigHSA")
...
# plt.plot(X,averTime[Arg.index('AA'),:],'c<-',label="AA")
plt.plot(X,averTime[Arg.index('NN'),:],'r^-',label="NN")
plt.legend(loc='upper left')
